...
export default async function PagePrice() {
  ...
  var progulyankoviKolyaski = data.filter(product => product?.attributes?.category === 'УНІВЕРСАЛЬНІ КОЛЯСКИ');
  var universalniKolyaski = data.filter(product => product?.attributes?.category === 'ПРОГУЛЯНКОВІ КОЛЯСКИ');
